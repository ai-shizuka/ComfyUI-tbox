...
class WD14Tagger {
    constructor() {
		if (!window.__wd14tagger__) {
			window.__wd14tagger__ = Symbol("__wd14tagger__");
		}
		this.symbol = window.__wd14tagger__;
	}
    getState(node) {
...
const wd14tagger = new WD14Tagger();

app.registerExtension({
	name: "tbox.WD14Tagger",
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		pysssss.addStatusTagHandler(nodeType);

		if (nodeData.name === "WD14Tagger|tbox") {
